...
static s32		g_ps2time_daylightsaving = -1;
...
int ps2time_init( void )
{
	...
	tm_time.tm_mon	= BCD2DEC(clock.month&0x7F) - 1; // RTC months range from 1 - 12
	...
	*T1_MODE	= Tn_MODE(0x02, 0, 0, 0, 0, 0x01, 0, 0x01, 0, 0);
	...
	g_ps2time_daylightsaving = 60;
	else
	g_ps2time_daylightsaving = 0;
...
{	
	time_t t = *timep + g_ps2time_timezone * 60 + g_ps2time_daylightsaving * 60;
